...
namespace Verlinea.ComboBoxTree
{
	...
	public class ComboBoxTree : UserControl
	{
		...
		{
            get { return this.tvTreeView.SelectedNode; }
			set 
			{
		...
		public ComboBoxTree() 
		{
            ...
            this.tvTreeView.Font = new System.Drawing.Font("ËÎÌו", 10F);
		...
		private void TreeViewNodeSelect(object sender, EventArgs e) 
		{
			if(this._absoluteChildrenSelectableOnly)
			{
				...
				{
					tbSelectedValue.Text = this.tvTreeView.SelectedNode.Text;//.FullPath.Replace(@"\", this._branchSeparator);
        ...
        public void setSelectNode(string name) {
            ...
            foreach (TreeNode node in tvTreeView.Nodes) {
                if(searchAndSetNode(node, name))
                    return;
            }
        }

        private bool searchAndSetNode(TreeNode node, string name) {
            if (node.Name == name)
            {
            ...
            else
            {
                ...
                {
                    if (searchAndSetNode(child, name))
                        return true;
                }
                return false;
            }
		...
		private void ComboBoxTree_Layout(object sender, System.Windows.Forms.LayoutEventArgs e)
		{
			...
			this.frmTreeView.Size = new Size(this.Width+30, 300);
			this.pnlTree.Size = this.frmTreeView.Size;
